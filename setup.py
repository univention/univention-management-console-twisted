...
class BuildI18N( cmd.Command ):
	...
	def run( self ):
		...
		po_dir = os.path.join( os.path.dirname( os.curdir ), 'univention/' )
...
setup( name = 'univention-umc',
	   description = 'Univention Management Console',
	   author = 'Univention GmbH',
	   author_email = 'packages@univention.de',
	   version = '1.0',
	   package_dir = { '' : '.' },
	   packages = [ 'univention', 'univention.umc', 'univention.umc.authentication', 'univention.umc.module', 'univention.umc.resources', 'univention.umc.server', 'univention.umc.util'],
	   scripts = [ 'scripts/umc-server', 'scripts/umc-module' ],
	   data_files = [],
